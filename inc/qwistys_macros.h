...
static void qwistys_default_log(qwistys_log_level_t level, const char* file, const char* function, int line, const char* tag, const char* fmt, ...) {
    const char* color;
    switch (level) {
    ...
    fprintf(output, "%s%s %s%s:%s:%d %s", color, QWISTYS_MACRONAME, QWISTYS_MAG, file, function, line, tag);
...
static void (*qwistys_log)(qwistys_log_level_t level, const char* file, const char* function, int line, const char* tag, const char* fmt, ...) = qwistys_default_log;
...
#    else
#        define QWISTYS_TODO_MSG(msg) QWISTYS_MSG(QWISTYS_LOG_LEVEL_WARN, QWISTYS_TAG_TODO, msg)
...
#define QWISTYS_ARRAY_ACCESS(array, index) (QWISTYS_ASSERT((index) >= 0 && (index) < QWISTYS_ARRAY_LEN(array)), array[index])
...
typedef void (*assert_handler_t)(const char* expr, const char* file, const char* function, int line);
...
static void default_assert_handler(const char* expr, const char* file, const char* function, int line) {
...
// Define the QWISTYS_ASSERT macro only if not in a release build
    ...
    #ifdef _MSC_VER
        // MSVC compiler
        #define QWISTYS_ASSERT(value) do { if (!(value)) (custom_assert_handler ? custom_assert_handler(#value, __FILE__, __func__, __LINE__) : __debugbreak()); } while (0)
    #elif defined(__GNUC__) || defined(__clang__)
        // GCC or Clang on Linux
        #define QWISTYS_ASSERT(value) do { if (!(value)) (custom_assert_handler ? custom_assert_handler(#value, __FILE__, __func__, __LINE__) : __builtin_trap()); } while (0)
    #else
        // Unknown compiler/platform
        #define QWISTYS_ASSERT(value) do { if (!(value)) (custom_assert_handler ? custom_assert_handler(#value, __FILE__, __func__, __LINE__) : abort()); } while (0)
    #endif
#else
...
void set_telemetry_handlers(telemetry_start_handler_t start_handler, telemetry_end_handler_t end_handler) {
...
#define QWISTYS_TELEMETRY_END() \
    if (telemetry_end_handler) { \
        ...
        double duration = (double)(end_time - telemetry_data.start_time) / CLOCKS_PER_SEC; \
